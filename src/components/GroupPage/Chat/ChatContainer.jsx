    ...
    <div
      ...
      <div className="flex flex-col gap-2.5 overflow-y-auto w-full h-[570px] mt-3 px-3">
        {groupChatData?.map(message => (
          <div
            key={message.id}
            className={`max-w-[286px] min-h-[40px] px-3 py-2.5 border flex-shrink-0 border-black rounded-lg break-words ${message.sender === userCode ? 'self-end bg-primary-200 text-neutral-50' : 'self-start bg-gray-100'}`}>
            {message.content}
          </div>
        ))}
      </div>
